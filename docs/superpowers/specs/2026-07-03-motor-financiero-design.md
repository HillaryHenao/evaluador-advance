...
8. **Costos** (todos negativos, indexados por IPC salvo que se indique lo contrario). Índices alineados por año calendario: para crecer el valor del año (k-1) hacia el año k se usa `IPC[k-1]`.
   - Arriendo: año 1 = `-arriendoAnual`; después `costo[k] = costo[k-1]*(1+IPC[k-1])`
   - Operación y optimización: `-ingresos[k] * operacionPctIngresos` (recalculado cada año sobre los ingresos de ese año, sin acumular)
   - CGM: tarifa `cgmPorKwh` que crece por IPC cada año (`tarifa[k] = tarifa[k-1]*(1+IPC[k-1])`), multiplicada por la generación del año actual
   ...
   - ICA: `-ventaEnergia[k] * ica`
   - **IVA** (no deducible): `(operacion[k]/2 + representacion[k] + mantenimiento[k]) * iva` — fácil de omitir por accidente, no aparece en ninguna otra sección del Excel
   ...
   - **Mantenimiento de tracker**: ocurre cada 5 años empezando en el año `anioBase+6` (2032), NO todos los años en 0 como parece a primera vista si solo se revisan los primeros años del Excel — hay que revisar toda la fila 23 hasta el final del horizonte. Montos ya calculados con las tarifas/FX fijos del Excel (literales, no dependen de los inputs del terreno): años 2032/2037/2042/2047/2052 = -35.117.188,69 / -41.107.231,39 / -48.119.013,38 / -56.326.815,75 / -65.934.647,24
   - **Reemplazo de inversores**: ocurre una sola vez, en el año `anioBase+16` (2042) = -250.052.057 (también literal, misma razón)
9. **Flujo de caja operativo**: `ingresos[k] + sum(costos[k])` — incluye TODOS los costos de arriba, incluyendo mantenimiento de tracker y reemplazo de inversores cuando aplican (ambos SÍ alimentan este total, a diferencia de lo que podría sugerir revisar solo los primeros años).
10. **CAPEX**: solo año 0 = `-capex` (Supuestos!D30). El Excel también tiene una fila 28 "CAPEX" separada con su propia lógica de reemplazo de inversores (HLOOKUP, año `anioBase+1+15`), pero esa fila alimenta un cálculo paralelo ("Flujo de caja libre/neto", filas 30/34/35) que **no** se usa para TIR/VPN/Payback — no confundir con el reemplazo de inversores del punto 8 (fila 22), que sí es necesario. La fila 38 (flujo del inversionista, base de todos los resultados) usa directamente la fila 26 (operativo) sin restar la fila 28 en ningún año salvo el año 0.
11. **Flujo del inversionista**: `flujoInversionista[0] = -capex`; `flujoInversionista[k] = flujoOperativo[k] * participacion` para k>=1 (participacion = 1 en el Excel, D29)
